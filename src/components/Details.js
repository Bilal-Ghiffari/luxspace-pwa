...
import { BreadcrumbDetail as Breadcrumb } from "../organisme/Breadcrumb";
import Header from "./Header";
import AsideMenu from "./AsideMenu";
import Footer from "./Footer";
import SemillarDetail from "./componentDetail/SemillarDetail";
...
export default function Details({ handleAddToCart, cart }) {
  ...
  const [currentImage, setCurrentImage] = useState(items.image1);
        ...
        <section className="container mx-auto">
          <div className="flex flex-wrap my-4 md:my-12">
            <div className="w-full md:hidden px-4">
              <h2 className="text-5xl font-semibold">{items.name}</h2>
              <span className="text-xl">{numberFormat(items.price)}</span>
            </div>
                ...
                <div className="thumbnail">
                  {dataImages.map((item, index) => (
                    <div className="px-2" key={index}>
                      <div
                        className={`item ${
                          currentImage === item.image && "selected"
                        }`}
                        onClick={() => setCurrentImage(item.image)}
                      >
                        <img
                          src={item.image ?? "Image sedang Di update"}
                          alt="front"
            ...
            <div className="flex-1 px-4 md:p-6">
              <h2 className="text-5xl font-semibold">{items.name}</h2>
              <p className="text-xl">{numberFormat(items.price)}</p>
              ...
              <p className="text-xl leading-7 mb-6">{items.description}</p>
            </div>
          </div>
        </section>
        <SemillarDetail Items={items} />
        <AsideMenu />
        <Footer />
      </>
    </>
