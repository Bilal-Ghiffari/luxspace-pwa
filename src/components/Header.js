    ...
    return (
        <header className={`${mode === 'light' ? 'absolute' : ''} w-full z-50 px-4`}>
            ...
            <div className="flex flex-stretch items-center">
                ...
                <ul
                    className={`fixed bg-pink-400 inset-0 flex flex-col items-center justify-center opacity-0 md:visible md:flex-row md:bg-transparent md:relative md:opacity-100 md:flex md:items-center ${hamburgerMenu ? "opacity-100 z-30 visible" : "invisible opacity-0"}`}
                    id="menu">
                    <li className="mx-3 py-6 md:py-0">
                    <a href="/" className={`text-black ${mode === 'light' ? 'md:text-white' : 'md:text-black'} hover:underline`}>Home</a>
                    </li>
                    <li className="mx-3 py-6 md:py-0">
                    <a href="/" className={`text-black ${mode === 'light' ? 'md:text-white' : 'md:text-black'} hover:underline`}>Catalog</a>
                    </li>
                    <li className="mx-3 py-6 md:py-0">
                    <a href="/" className={`text-black ${mode === 'light' ? 'md:text-white' : 'md:text-black'} hover:underline`}>Delivery</a>
                    </li>
                    <li className="mx-3 py-6 md:py-0">
                    <Link index to="/profile" className={`text-black ${mode === 'light' ? 'md:text-white' : 'md:text-black'} hover:underline`}>Profile</Link>
                    </li>
                </ul>
                </div>
                <div className="w-auto">
                <ul className="items-center flex">
                    ...
                    type='button'
                        className={`flex z-50 items-center justify-center w-8 h-8 text-black md:text-white focus:outline-none ${hamburgerMenu ? "fixed top-10 right-6" : "relative"}`}
                    ...
                    <li className="ml-6">
                    <Link id="header-cart" aria-label='logo-cart' className={`flex items-center justify-center w-8 h-8 text-black ${mode === 'light' ? 'md:text-white' : 'md:text-black'}`}
                        to="/cart">
                            ...
                            {cart && cart.length !== 0 && (
                                <g className="text-pink-400">
                                    <circle
                                        ...
                                        // cy="4.89023"
                                        r="4"
                                        className="fill-current dot animate-pulse"
                                    />
                                </g>
                            )}
                        </svg>
